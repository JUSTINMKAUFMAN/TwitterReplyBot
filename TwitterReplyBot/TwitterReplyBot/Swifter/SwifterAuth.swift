...
public extension Swifter {
     ...
     */
        ...
        func authorize(withCallback callbackURL: URL,
            ...
            postOAuthRequestToken(with: callbackURL, success: { token, response in
     ...
     */
        ...
        func authorizeSSO(success: SSOTokenSuccessHandler?, failure: FailureHandler? = nil) {
            ...
            let urlScheme = "twitter-\(client.consumerKey)"

            let nc = NotificationCenter.default
            swifterCallbackToken = nc.addObserver(forName: .swifterSSOCallback, object: nil, queue: .main) { notification in
                self.swifterCallbackToken = nil
    ...
    func authorizeAppOnly(success: TokenSuccessHandler?, failure: FailureHandler?) {
        postOAuth2BearerToken(success: { json, response in
    ...
    func postOAuth2BearerToken(success: JSONSuccessHandler?, failure: FailureHandler?) {
        ...
        jsonRequest(path: path, baseURL: .oauth, method: .POST, parameters: parameters, success: success, failure: failure)
    }

    func invalidateOAuth2BearerToken(success: TokenSuccessHandler?, failure: FailureHandler?) {
        let path = "oauth2/invalidate_token"

        jsonRequest(path: path, baseURL: .oauth, method: .POST, parameters: [:], success: { json, response in
    ...
    func postOAuthRequestToken(with callbackURL: URL, success: @escaping TokenSuccessHandler, failure: FailureHandler?) {
        ...
        client.post(path, baseURL: .oauth, parameters: parameters, uploadProgress: nil, downloadProgress: nil, success: { data, response in
