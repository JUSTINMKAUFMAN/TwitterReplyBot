...
public struct HMAC {
    internal static func sha1(key: Data, message: Data) -> Data? {
        ...
        if key.count > 64 { key = SHA1(Data(bytes: key)).calculate() }
        if key.count < 64 { key = key + [UInt8](repeating: 0, count: 64 - key.count) }

        var opad = [UInt8](repeating: 0x5C, count: 64)
        for (idx, _) in key.enumerated() { opad[idx] = key[idx] ^ opad[idx] }
        var ipad = [UInt8](repeating: 0x36, count: 64)
        for (idx, _) in key.enumerated() { ipad[idx] = key[idx] ^ ipad[idx] }
